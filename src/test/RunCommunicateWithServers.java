...
//Initializing servers
    //   Server mazeGeneratingServer = new Server(5400, 200, new ServerStrategyGenerateMaze());
        Server solveSearchProblemServer = new Server(5401, 1000, new ServerStrategySolveSearchProblem());
        Server solveSearchProblemServer2 = new Server(5402, 1000, new ServerStrategySolveSearchProblem());

//Server stringReverserServer = new Server(5402, 1000, new
       // ServerStrategyStringReverser());
//Starting servers
        solveSearchProblemServer.start();
        solveSearchProblemServer2.start();
...
//Stopping all servers
      //  mazeGeneratingServer.stop();
        ...
        solveSearchProblemServer2.stop();
...
//Print Maze Solution retrieved from the server
        ...
        try {
            Client client = new Client(InetAddress.getLocalHost(), 5402, new
                    IClientStrategy() {
                        @Override
                        public void clientStrategy(InputStream inFromServer,OutputStream outToServer,Maze maze2) {
                            try {
                                ObjectOutputStream toServer = new ObjectOutputStream(outToServer);
                                ObjectInputStream fromServer = new ObjectInputStream(inFromServer);
                                toServer.flush();
